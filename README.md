...
既然 Vim/NeoVim 本身在不停的进化，一些固有的工作流程是值得我们拿出来重新检讨反思，比如为啥同样是编辑器的 vscode 可以很好的把这类事情打理清楚，让大部分任务都能高效的使用 vscode 构建系统完成，而 Vim 却远远做不到这种程度？

任务构建这件事情，是否能在 Vim 下面，也做得根 vscode 一样漂亮呢？

因此，本插件目的就在于把 `neomake` （构建部分）以及 `quickrun` 还有 `dispatch` 之类的插件全部废弃掉，进化到更科学现代的构建方法上来。
