...
这是一个经过思考的，合理的任务构建系统，用于弥补 Vim 长期以来缺乏类似 vscode 这样的任务系统的不足：

- 简单高效，高度定制化
- 支持针对项目的局部任务配置
- 支持全局任务配置
- 丰富的运行模式：quickfix，内置终端，外部终端，传统 `!` 运行
- 为 Vim/NeoVim 提供一致的使用体验
- 针对文件类型相关的任务模式

我看过很多 `.vimrc` 配置，大部分在关于如何编译和运行项目这方面，都在用一些非常原始的方法，这么多年下来，仍然缺少一套系统化的解决方案。

可能原教旨主义者会说：“Vim 只是个编辑器”，“这些事情放在外面做”，“tmux 用来干嘛的？” 之类的观点。Vim 社区里很多人总是喜欢教别人，这个是设计出来干嘛的，不是干嘛的。

然而时代在进步，Vim/NeoVim 本身都在不停的进化，人也应该一起进化才对。固有的工作流程都值得我们拿出来重新检讨一下，比如为啥同样是编辑器的 vscode 可以很好的把这类事情打理清楚，让大部分任务都能高效的使用 vscode 构建系统完成，而 Vim 却远远做不到这种程度？

既然基于 vscode 的工作流，并不是十分依赖 tmux 和外部终端的存在，那么基于 vim 的工作流是不是也可以做到这样呢？

所以，本插件目的就在于把 `neomake` （构建部分）以及 `quickrun` 还有 `dispatch` 之类的插件全部废弃掉，进化到更科学现代的构建方法上来。
...
有的项目用 cmake 构建，有的项目用 ninjia 构建，基于项目目录的局部配置机制，让你可以定义同样一个任务名称，绑定同样一个快捷键，却在不同的项目里执行特定的构建命令。
